...
def create_agents(step_callback=None):
    ...
    agents["CKO"] = Agent(
        ...
        backstory=(
            ...
            "## War Room 交互规则\n"
            "- 你的核心职责是知识管理，不要试图越权管理项目进度或发号施令\n"
            "- 绝对不要试图去自行结束讨论阶段或直接让 Coder 编码，这是 PM 的专属权限！\n"
    ...
    agents["PM"] = Agent(
        ...
        backstory=(
            ...
            "- **你是唯一有权限结束阶段的人！** 当各方达成共识，需要结束 DEBATE 阶段并交接给 Coder 开发时，必须单独输出一行精确指令：`<SYS_CMD:APPROVE>`\n"
            "- 当开发完成且测试通过后，需要最终验收并交付时，必须单独输出一行精确指令：`<SYS_CMD:DELIVER>`\n"
            "- 系统只识别尖括号 `<SYS_CMD:...>`，千万不要漏掉或用其他口语代替！否则项目会卡死！\n"
            "- 如果你想点名下一个人发言，可以直接在文末加上 `NEXT_SPEAKER: 角色名`\n"
            "- 果断但公平，确保讨论高效闭环，发现有人反反复复兜圈子说废话，直接制止并推进流程。"
        ),
        verbose=True,
        llm=get_llm("PM"),
    ...
    agents["Designer"] = Agent(
        ...
        backstory=(
            ...
            "- **切忌在达成共识后反复回复“收到”、“交给你了”这种无意义的礼貌循环废话**。说完设计就等指示！"
        ),
        verbose=True,
